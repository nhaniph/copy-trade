...
DAX=GER40, DOW=US30, NQ=US100, FTSE=UK100, WTI=Oil, DXY=Dollar index.
...
def classify_batch(messages: list[dict]) -> list[dict]:
    """
    Classify a batch of messages together.
    messages: list of {index, content, timestamp} dicts
    Returns list of idea objects with grouped messages.
    """
    numbered = "\n".join(
    ...
    response = client.messages.create(
        ...
        messages=[{"role": "user", "content": f"Here are Tom's messages:\n\n{numbered}"}],
    )
    ...
    try:
        ...
        print(f"  [Batch] Could not parse response: {text[:300]}")
        return []
